...
class TaskCubit extends Cubit<TaskCubitStates>{
  ...
  // void getTask({required task_id}){
  //   emit(());
  //   FirebaseFirestore.instance.collection("tasks").doc("$task_id").get().then((value) {
  //     model = TaskModel.fromJson(value.data());
  ...
  void UpdateTaskID(id){
    emit(UpdateIDLoadingState());
    //get the task id uppdated
    FirebaseFirestore.instance.collection("tasks").doc(id).update({"task_id" : id }).then((value) => emit(UpdateIDSuccessState()))
